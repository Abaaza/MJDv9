...
export async function uploadAndMatchWithBlob(req: Request, res: Response): Promise<void> {
  try {
    ...
    const { url: blobUrl, fileId } = await blobStorage.uploadBOQFile(
      req.user.id,
...
export async function exportResultsWithBlob(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (process.env.STORE_RESULTS_IN_BLOB === 'true') {
      const resultUrl = await blobStorage.uploadResultFile(
        jobId,
        exportBuffer,
        job.fileName
      );

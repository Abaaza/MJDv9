    ...
    if (LambdaProcessorService.isLambdaEnvironment() && lambdaProcessor.shouldProcessSynchronously(preparedItems.length)) {
      ...
      try {
        ...
        await convex.mutation(api.priceMatching.updateJobStatus, {
          ...
          error: `File is too large for Lambda processing (${preparedItems.length} items). Please try: 1) Upload a smaller file (under 2000 items), 2) Split your file into smaller chunks, or 3) Contact support for batch processing options.`
        });
        
        throw new Error(`File too large for Lambda environment. Please upload files with less than 2000 items or split your file into smaller chunks.`);
      }
      
      // Console log removed for performance
    } else if (LambdaProcessorService.isLambdaEnvironment()) {
      // In Lambda but file is too large (>2000 items)
      ...
      await convex.mutation(api.priceMatching.updateJobStatus, {
        jobId: jobId,
        status: 'failed' as any,
        error: `This file contains ${preparedItems.length} items, which exceeds the Lambda processing limit of 2000 items. Please split your file into smaller chunks (max 2000 items each) and process them separately.`,
        progress: 0,
        progressMessage: 'File too large for serverless processing'
      });
      
      throw new Error(`File too large (${preparedItems.length} items). Lambda can only process files with up to 2000 items. Please split your file into smaller chunks.`);

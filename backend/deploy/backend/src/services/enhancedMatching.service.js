...
exports.enhancedMatchingService = exports.EnhancedMatchingService = void 0;
const fuzzball = __importStar(require("fuzzball"));
class EnhancedMatchingService {
    ...
    static getUnitConversionFactor(fromUnit, toUnit) {
        ...
        for (const conv of this.unitConversions) {
            if (conv.from.includes(fromNorm))
                fromConversion = conv;
            if (conv.from.includes(toNorm))
                toConversion = conv;
        }
    ...
    static calculateScore(queryText, itemText, options = {}) {
        // Base fuzzy match score
        let score = fuzzball.token_set_ratio(queryText, itemText);
        // Apply bonuses
        if (options.unitMatch)
            score += 10;
        if (options.categoryMatch)
            score += 15;
        ...
        if (options.method === 'exact')
            score = 100;
        if (options.method === 'code' && score > 80)
            score = 95;
        return Math.min(score, 100);
    }
    static enhancedFuzzyMatch(query, priceItems, limit = 5, contextHeaders) {
        ...
        for (const item of priceItems) {
            ...
            if (contextHeaders && contextHeaders.length > 0) {
                const contextMatch = contextHeaders.some(header => processedItem.includes(header.toLowerCase()) ||
            ...
            if (item.category) {
                ...
                const categoryMatch = categoryKeywords.some(k => processedQuery.includes(k.toLowerCase()));
                if (categoryMatch)
                    score += 15;
    ...
    static async multiStageMatch(query, priceItems, options = {}) {
        const { useExactMatch = true, useCodeMatch = true, useFuzzyMatch = true, useSemanticMatch = false, limit = 5 } = options;
        const allMatches = [];
        const processedQuery = this.preprocessText(query);
        // Stage 1: Exact match
        if (useExactMatch) {
...
exports.EnhancedMatchingService = EnhancedMatchingService;
EnhancedMatchingService.constructionAbbreviations = [
...
EnhancedMatchingService.constructionKeywords = {
...
EnhancedMatchingService.unitConversions = [
    // Area conversions
...
exports.enhancedMatchingService = new EnhancedMatchingService();

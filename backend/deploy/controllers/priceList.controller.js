...
export async function getPriceListStats(req, res) {
    try {
...
export async function getAllPriceItems(req, res) {
    try {
...
export async function createPriceItem(req, res) {
    try {
        if (!req.user) {
...
export async function deactivatePriceItem(req, res) {
    try {
        if (!req.user) {
            res.status(401).json({ error: 'Not authenticated' });
            return;
        ...
        const { id } = req.params;
        await convex.mutation(api.priceItems.deleteItem, { id: toConvexId(id) });
        // Create activity log
        await convex.mutation(api.activityLogs.create, {
            userId: toConvexId(req.user.id),
...
export async function searchPriceItems(req, res) {
    try {
        if (!req.user) {
            res.status(401).json({ error: 'Not authenticated' });
            return;
...
export async function deleteAllPriceItems(req, res) {
    try {
        if (!req.user) {
            res.status(401).json({ error: 'Not authenticated' });
            return;

...
export async function uploadForProject(req, res) {
    try {
        ...
        const jobId = await convex.mutation(api.priceMatching.createJob, {
            ...
            projectId: toConvexId(projectId),
        ...
        const batchResults = await processBatch(allItems, 10, // Process 10 items at a time
        async (item) => {
...
export async function uploadAndMatchForProject(req, res) {
    try {
        if (!req.file) {
            res.status(400).json({ error: 'No file uploaded' });
            return;
...
export async function exportProjectResults(req, res) {
    try {
...
export async function getProjectJobs(req, res) {
    try {
...
export async function linkJobToProject(req, res) {
    try {
        const { jobId } = req.params;
...
export async function unlinkJobFromProject(req, res) {
    try {
        const { jobId } = req.params;
        if (!req.user) {
            res.status(401).json({ error: 'Not authenticated' });

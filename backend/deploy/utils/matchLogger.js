export class MatchLogger {
    static logs = new Map();
    static MAX_LOGS_PER_JOB = 1000;
    /**
     * Log a detailed match result
     */
    static logMatch(jobId, itemIndex, originalDescription, matchResult, scoreBreakdown, processingTime, cacheHit = false, apiCalls = 0) {
    ...
    static getJobStats(jobId) {
        ...
        for (const log of logs) {
            ...
            if (log.cacheHit)
                cacheHits++;
            if (log.scoreBreakdown.confidence < 0.3)
                stats.lowConfidenceMatches++;
            // Update score distribution
            const conf = log.scoreBreakdown.confidence;
            if (conf <= 0.2)
                stats.scoreDistribution['0-0.2']++;
            else if (conf <= 0.4)
                stats.scoreDistribution['0.2-0.4']++;
            else if (conf <= 0.6)
                stats.scoreDistribution['0.4-0.6']++;
            else if (conf <= 0.8)
                stats.scoreDistribution['0.6-0.8']++;
            else
                stats.scoreDistribution['0.8-1.0']++;
        }
     ...
     */
    static getProblematicMatches(jobId, threshold = 0.3) {
        const logs = this.logs.get(jobId) || [];
     ...
     */
    static clearJobLogs(jobId) {
        this.logs.delete(jobId);
    }
}
//# sourceMappingURL=matchLogger.js.map
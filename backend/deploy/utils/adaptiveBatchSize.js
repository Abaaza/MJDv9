...
export class AdaptiveBatchSizer {
    static performanceHistory = new Map();
    static currentBatchSizes = new Map();
    static HISTORY_SIZE = 10;
    static ADJUSTMENT_THRESHOLD = 0.2; // 20% performance change triggers adjustment
    /**
     * Get optimal batch size based on recent performance
     */
    static getOptimalBatchSize(jobId, method) {
     ...
     */
    static recordBatchPerformance(jobId, method, batchSize, duration, itemsProcessed) {
        const key = `${jobId}-${method}`;
     ...
     */
    static calculatePerformanceTrend(history) {
        if (history.length < 2)
            return 0;
     ...
     */
    static getPerformanceStats(jobId, method) {
        const key = `${jobId}-${method}`;
        const history = this.performanceHistory.get(key);
...
//# sourceMappingURL=adaptiveBatchSize.js.map
...
export class MatchingService {
    ...
    async matchItemWithEmbedding(description, method, preGeneratedEmbedding, providedPriceItems, contextHeaders) {
        ...
        const itemsWithEmbeddings = priceItems.filter(item => item.embedding && item.embeddingProvider === method.toLowerCase());
    ...
    async localMatch(description, priceItems, contextHeaders) {
        ...
        matches.sort((a, b) => {
            ...
            if (Math.abs(b.score - a.score) < 10 && queryUnit) {
                ...
                if (aHasUnitMatch && !bHasUnitMatch)
                    return -1;
                if (!aHasUnitMatch && bHasUnitMatch)
                    return 1;
            }
    ...
    async cohereMatch(description, priceItems, contextHeaders) {
        ...
        if (contextHeaders && contextHeaders.length > 0) {
            ...
            if (contextHeaders[0])
                queryText += ` ${contextHeaders[0]}`;
            if (contextHeaders[1])
                queryText += ` ${contextHeaders[1]}`;
        }
        ...
        // Score all items
        const scoredItems = await Promise.all(priceItems.map(async (item) => {
        ...
        const validMatches = scoredItems.filter(m => m !== null);
     ...
     */
    async generateBatchEmbeddings(descriptions, method) {
        await this.ensureClientsInitialized();
        const embeddings = new Map();

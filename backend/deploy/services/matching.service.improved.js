...
export class ImprovedMatchingService {
    ...
    async cohereMatch(description, priceItems, contextHeaders) {
        ...
        try {
            ...
            const embeddingResponse = await withRetry(() => this.cohereClient.embed({
                texts: [description],
    ...
    async openaiMatch(description, priceItems, contextHeaders) {
        ...
        try {
            // Get embedding for the description
            const embeddingResponse = await withRetry(() => this.openaiClient.embeddings.create({
                input: description,
    ...
    async getItemEmbeddings(items, provider) {
    ...
    async generateBatchEmbeddings(items, provider) {
        ...
        try {
            if (provider === 'cohere' && this.cohereClient) {
                ...
                const embeddings = Array.isArray(response.embeddings)
                    ? response.embeddings
                    : response.embeddings.float || response.embeddings.int8 || response.embeddings.uint8 || response.embeddings.ubinary || response.embeddings.binary || [];
                return embeddings;
            }
    ...
    async findMatches(description, priceItems, limit = 10, method = 'ADVANCED') {
        try {
            const matches = await EnhancedMatchingService.multiStageMatch(description, priceItems, { limit });
...
//# sourceMappingURL=matching.service.improved.js.map
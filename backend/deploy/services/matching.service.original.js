...
export class MatchingService {
    ...
    async ensureClientsInitialized() {
        if (!this.cohereClient || !this.openaiClient) {
            ...
            const settings = await withRetry(() => this.convex.query(api.applicationSettings.getAll), {
                maxAttempts: 3,
                delayMs: 1000,
    ...
    async getPriceItems() {
        ...
        // Load from database
        const items = await withRetry(() => this.convex.query(api.priceItems.getActive), {
            maxAttempts: 3,
            delayMs: 1000,
            onRetry: (error, attempt) => {
    ...
    async localMatch(description, priceItems, contextHeaders) {
        ...
        for (const item of priceItems) {
            ...
            const constructionScore = ConstructionPatternsService.calculateConstructionScore(queryFeatures, itemFeatures);
...
//# sourceMappingURL=matching.service.original.js.map
...
export class MatchingService {
     ...
     */
    async generateBatchEmbeddings(priceItems, provider = 'cohere') {
        ...
        for (let i = 0; i < batches.length; i++) {
            ...
            try {
                if (provider === 'cohere') {
                    const response = await withRetry(() => this.cohereClient.embed({
                        texts,
                    ...
                    const response = await withRetry(() => this.openaiClient.embeddings.create({
    ...
    async matchItem(description, method, priceItems, contextHeaders) {
        ...
        if (!priceItems) {
            ...
            try {
                priceItems = await withRetry(() => this.convex.query(api.priceItems.getActive), {
                    maxAttempts: 3,
                    delayMs: 1000,
                    onRetry: (error, attempt) => {
    ...
    async localUnitMatch(description, priceItems, contextHeaders) {
        ...
        for (const item of priceItems) {
            ...
            if (contextHeaders && item.category) {
                const categoryMatch = contextHeaders.some(h => item.category.toLowerCase().includes(h.toLowerCase().split(' ')[0]));
    ...
    async cohereMatch(description, priceItems, generateEmbeddings = false, contextHeaders) {
        ...
        const embeddings = Array.isArray(embeddingResponse.embeddings)
            ...
            : embeddingResponse.embeddings.float || embeddingResponse.embeddings.int8 || embeddingResponse.embeddings.uint8 || embeddingResponse.embeddings.ubinary || embeddingResponse.embeddings.binary || [];
        const queryEmbedding = embeddings[0];
        // Get embeddings from cache or database
        let itemsWithEmbeddings = [];
    ...
    async openaiMatch(description, priceItems, generateEmbeddings = false, contextHeaders) {
    ...
    async hybridMatch(description, priceItems, contextHeaders) {
    ...
    async advancedMatch(description, priceItems, contextHeaders) {
        ...
        if (codeMatch) {
            ...
            const codeMatches = priceItems.filter(item => item.code && item.code.toUpperCase().includes(code));
        ...
        if (patterns.length > 0) {
            ...
            const patternMatches = EnhancedMatchingService.enhancedFuzzyMatch(description, priceItems.filter(item => !stages.some(s => s.item._id === item._id)), // Exclude already matched
            20, contextHeaders);
        ...
        if (stages.length < 5) {
            const fuzzyMatches = EnhancedMatchingService.enhancedFuzzyMatch(description, priceItems, 10, contextHeaders);
    ...
    extractTechnicalSpecs(description) {
        ...
        const dimensionPattern = /\d+(?:\.\d+)?\s*[xÃ—]\s*\d+(?:\.\d+)?\s*(?:mm|cm|m|in|ft)?/gi;
        const dimensions = description.match(dimensionPattern);
        if (dimensions)
            specs.push(...dimensions);
        ...
        if (measurements)
            specs.push(...measurements.filter(m => !dimensions?.includes(m)));
        ...
        if (standards)
            specs.push(...standards);
        return [...new Set(specs)]; // Remove duplicates
    }
    extractWorkTypes(description) {
        const workTypes = [];
    ...
    async hybridCategoryMatch(description, priceItems, contextHeaders) {
        ...
        try {
            ...
            if (detectedCategories.size > 0) {
                const categoryFilteredItems = priceItems.filter(item => item.category && detectedCategories.has(item.category.toLowerCase()));
        ...
        const bestResult = results.reduce((best, current) => current.result.confidence > best.result.confidence ? current : best);
        return {
            ...bestResult.result,
            method: 'HYBRID_CATEGORY'
        };
    }
    async getTopMatches(description, method, priceItems, topN = 5, contextHeaders) {
...
//# sourceMappingURL=matching.service.backup.js.map
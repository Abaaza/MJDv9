...
export class JobProcessorService extends EventEmitter {
    ...
    CONVEX_BATCH_SIZE = 50; // Update Convex every 50 items
    // Rate limiting
    lastConvexUpdate = 0;
    MIN_UPDATE_INTERVAL = 5000; // Minimum 5 seconds between Convex updates
    CONVEX_MUTATION_DELAY = 100; // 100ms between individual mutations
    CONVEX_BATCH_DELAY = 2000; // 2 seconds between batches
    constructor() {
        super();
        // Start the processor
        this.startProcessor();
        ...
        try {
            ...
            for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
                ...
                if (batchIndex === totalBatches - 1 || batchIndex % 10 === 9) {
                    ...
                    this.emitLog(jobId, 'success', `Batch ${batchNumber}/${totalBatches} done - ${successCount}/${itemsWithQuantities} matched so far`);
                }

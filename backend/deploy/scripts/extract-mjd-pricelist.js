...
async function extractFromCategorySheets() {
    try {
                ...
                for (let j = 1; j <= Math.min(10, worksheet.columnCount); j++) {
                    ...
                    if (cellValue === 'description') {
                        ...
                        for (let k = j + 1; k <= Math.min(j + 10, worksheet.columnCount); k++) {
                            ...
                            if (headerValue.includes('quant'))
                                quantCol = k;
                            else if (headerValue.includes('unit'))
                                unitCol = k;
                            else if (headerValue.includes('rate'))
                                rateCol = k;
                        }
                        break;
                    }
            ...
            for (let i = dataStartRow; i <= worksheet.rowCount; i++) {
                ...
                if (descCell) {
                    if (typeof descCell === 'object' && 'richText' in descCell) {
                        ...
                        description = descCell.result?.toString() || '';
                    }
                    else {
                ...
                let rateValue = row.getCell(rateCol).value;
        ...
        // Save as JSON
        await promises_1.default.writeFile('mjd_pricelist_extracted.json', JSON.stringify(allItems, null, 2));
        ...
        await promises_1.default.writeFile('mjd_pricelist_extracted.csv', csvHeader + csvRows.join('\n'));
        ...
        await promises_1.default.writeFile('mjd_pricelist_for_import.json', JSON.stringify(importData, null, 2));
...
//# sourceMappingURL=extract-mjd-pricelist.js.map
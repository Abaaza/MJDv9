    ...
    try {
        ...
        for (const sheetName of categorySheets) {
            ...
            for (let i = 1; i <= Math.min(30, worksheet.rowCount); i++) {
                ...
                if (foundDesc && foundRate)
                    break;
            }
            // Default columns if not found
            if (descCol === -1)
                descCol = 2; // Usually column B
        ...
        // Save outputs
        await promises_1.default.writeFile('mjd_complete_pricelist.json', JSON.stringify(allItems, null, 2));
        ...
        await promises_1.default.writeFile('mjd_complete_pricelist.csv', csvHeader + csvRows.join('\n'));
        ...
        await promises_1.default.writeFile('mjd_pricelist_import_ready.json', JSON.stringify(importData, null, 2));
...
// Run extraction
extractAllSheets();
//# sourceMappingURL=extract-all-sheets.js.map
...
function generateKeywords(description, category, subcategory = "") {
    ...
    if (descLower.includes('supply'))
        keywords.add('provide');
    if (descLower.includes('install'))
        keywords.add('fix');
    if (descLower.includes('laying'))
        keywords.add('lay');
    if (descLower.includes('casting'))
        keywords.add('cast');
    if (descLower.includes('excavation'))
        keywords.add('excavate');
    return Array.from(keywords);
}
async function extractPricelist() {
    try {
        ...
        for (let j = 1; j <= worksheet.columnCount; j++) {
            ...
            if (cellValue.includes('item') && itemCol === 0)
                itemCol = j;
            else if ((cellValue.includes('description') || cellValue.includes('particular')) && descCol === 0)
                descCol = j;
            else if (cellValue.includes('unit') && unitCol === 0)
                unitCol = j;
            else if ((cellValue.includes('rate') || cellValue.includes('price')) && rateCol === 0)
                rateCol = j;
        }
        ...
        // Save as JSON
        await promises_1.default.writeFile('extracted_pricelist.json', JSON.stringify(items, null, 2));
        ...
        await promises_1.default.writeFile('extracted_pricelist.csv', csvHeader + csvRows.join('\n'));
...
// Run the extraction
extractPricelist();
//# sourceMappingURL=extract-pricelist.js.map
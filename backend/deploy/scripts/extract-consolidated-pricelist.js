...
function extractNumericValue(cell) {
    ...
    if (typeof value === 'object' && 'result' in value) {
        const num = parseFloat(value.result?.toString() || '');
...
function consolidateDescription(desc) {
    return desc
        .toLowerCase()
...
async function extractConsolidatedPricelist() {
    try {
        ...
        for (const sheetName of categorySheets) {
            ...
            for (let i = 1; i <= Math.min(30, worksheet.rowCount); i++) {
                ...
                for (let j = 1; j <= Math.min(15, worksheet.columnCount); j++) {
                    ...
                    if (cellValue === 'unit')
                        unitCol = j;
                    if (cellValue === 'rate' || cellValue === 'price')
                        rateCol = j;
                }
        ...
        consolidatedItems.forEach(item => {
            ...
            const isPriority = priorityKeywords.some(kw => item.description.toLowerCase().includes(kw) ||
        ...
        // Save outputs
        await promises_1.default.writeFile('mjd_consolidated_pricelist.json', JSON.stringify(finalItems, null, 2));
        await promises_1.default.writeFile('mjd_consolidated_import.json', JSON.stringify(finalItems.map(item => ({
            code: item.code,
            ref: item.ref,
            description: item.description,
            category: item.category,
        ...
        await promises_1.default.writeFile('mjd_consolidated_pricelist.csv', csvHeader + csvRows.join('\n'));
        // Summary
        console.log('\nSummary by category:');
...
//# sourceMappingURL=extract-consolidated-pricelist.js.map
...
& ssh -i $sshKey -o StrictHostKeyChecking=no ec2-user@54.82.88.31 "grep -i 'listening\|started\|server.*running' /tmp/backend.log || echo 'No server start message found'"

Write-Host "`n2. Checking all node processes:" -ForegroundColor Cyan
& ssh -i $sshKey -o StrictHostKeyChecking=no ec2-user@54.82.88.31 "ps aux | grep node"

Write-Host "`n3. Checking all listening ports:" -ForegroundColor Cyan
& ssh -i $sshKey -o StrictHostKeyChecking=no ec2-user@54.82.88.31 "sudo netstat -tlpn | grep -E '(LISTEN|node)'"

Write-Host "`n4. Looking for errors in backend log:" -ForegroundColor Cyan
& ssh -i $sshKey -o StrictHostKeyChecking=no ec2-user@54.82.88.31 "grep -i 'error\|fail\|exception' /tmp/backend.log | tail -10"

Write-Host "`n5. Checking server.js file for listen call:" -ForegroundColor Cyan
& ssh -i $sshKey -o StrictHostKeyChecking=no ec2-user@54.82.88.31 "grep -n 'listen' /home/ec2-user/app/backend/dist/server.js | tail -5"

Write-Host "`n6. Starting a simple test server to verify connectivity:" -ForegroundColor Cyan
& ssh -i $sshKey -o StrictHostKeyChecking=no ec2-user@54.82.88.31 "pkill -f test-server.js; cd /home/ec2-user && echo 'const http = require(""http""); http.createServer((req,res) => res.end(JSON.stringify({status:""ok""}))).listen(5000, () => console.log(""Test server on 5000""));' > test-server.js && node test-server.js > test-server.log 2>&1 & sleep 2 && curl http://localhost:5000"
